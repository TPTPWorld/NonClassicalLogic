...
tff(a1, axiom, {$obligatory}(processDataLawfully)).

%--- If data is not processed lawfully it has to be erased
tff(a2, axiom, {$obligatory}(~processDataLawfully),eraseData).

%--- Data is actually not processed lawfully
tff(situationalAx, localAxiom, ~processDataLawfully). 

%--- Is it obligatory to delete the data?
tff(c1, conjecture, {$obligatory}(eraseData)).

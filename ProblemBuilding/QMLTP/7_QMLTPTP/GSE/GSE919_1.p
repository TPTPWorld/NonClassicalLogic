%------------------------------------------------------------------------------
% File     : GSE919_1 : TPTP v9.0.0. Released v9.0.0.
% Domain   : Set Theory
% Problem  : in(A,B) => ((in(C,B) & A!=C) | intsctn(uno_pair(A,C),B) = sgtn(A))
% Version  : [QMLTP] axioms.
% English  : Goedel encoding of the TPTP SET919+1 problem

% Refs     : [Goe69] Goedel (1969), An Interpretation of the Intuitionistic
%          : [RO12]  Raths & Otten (2012), The QMLTP Problem Library for Fi
% Source   : [QMLTP]
% Names    : GSE919+1 [QMLTP]

% Status   : Unknown
% Rating   : ? v9.0.0
% Syntax   : Number of formulae    :   27 (   0 unt;   6 typ;   0 def)
%            Number of atoms       :  394 (   0 equ)
%            Maximal formula atoms :    2 (  18 avg)
%            Number of connectives :  216 (   3   ~;   5   |;  18   &)
%                                         (   0 <=>;  30  =>;   0  <=;   0 <~>)
%                                         ( 160 {}@;   0 {#};   0 {.})
%            Maximal formula depth :    3 (   2 avg)
%            Maximal term depth    :    3 (   1 avg)
%            Number of FOOLs       :  160 ( 160 fml;   0 var)
              ...
              @ (! [C: $i] :
                   ...
                   @ (( ( {$box} @ (in(A,B)) )
                     => ( ( ( {$box} @ (in(C,B)) )
                          & ( {$box}
                            @ (~ ( {$box} @ (qmltpeq(A,C)) )) ) )
                        | ( {$box} @ (qmltpeq(set_intersection2(unordered_pair(A,C),B),singleton(A))) ) ) )) )) )) )) ) ).

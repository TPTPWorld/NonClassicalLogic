...
% English  : Goedel encoding of the TPTP SET016+1 problem

% Refs     : [Goe69] Goedel (1969), An Interpretation of the Intuitionistic
%          : [RO12]  Raths & Otten (2012), The QMLTP Problem Library for Fi
% Source   : [QMLTP]
% Names    : GSE016+1 [QMLTP]
%          : GSE016_1.047_s5_decreasing_flexible.p [QMLTP/4_NX0_SEMs]

% Status   : Unknown
% Rating   : ? v9.0.0
% Syntax   : Number of formulae    :  116 (   0 unt;  32 typ;   0 def)
%            Number of atoms       : 1314 (   0 equ)
%            Maximal formula atoms :    6 (  15 avg)
%            Number of connectives :  703 (   8   ~;   5   |;  68   &)
%                                         (   0 <=>;  87  =>;   0  <=;   0 <~>)
%                                         ( 535 {}@;   0 {#};   0 {.})
%            Maximal formula depth :    5 (   2 avg)
%            Maximal term depth    :    4 (   1 avg)
%            Number of FOOLs       :  535 ( 535 fml;   0 var)
...
%            Number of variables   :  201 (; 196   !;   5   ?; 201   :)
...
tff(ordered_pair_determines_components1,conjecture,
    ( {$box}
    @ (! [W: $i] :
         ( {$box}
         @ (! [X: $i] :
              ...
              @ (! [Y: $i] :
                   ...
                   @ (! [Z: $i] :
                        ( {$box}
                        @ (( ( ( {$box} @ (qmltpeq(ordered_pair(W,X),ordered_pair(Y,Z))) )
                             & ( {$box} @ (member(W,universal_class)) ) )
                          => ( {$box} @ (qmltpeq(W,Y)) ) )) )) )) )) )) ) ).

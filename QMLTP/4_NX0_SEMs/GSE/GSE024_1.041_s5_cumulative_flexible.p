...
% Source   : [QMLTP]
% Names    : GSE024+1 [QMLTP]
%          : GSE024_1.041_s5_cumulative_flexible.p [QMLTP/4_NX0_SEMs]
...
%            Number of connectives :  698 (   8   ~;   5   |;  67   &)
%                                         (   0 <=>;  87  =>;   0  <=;   0 <~>)
%                                         ( 531 {}@;   0 {#};   0 {.})
...
tff(set_in_its_singleton,conjecture,
    ( {$box}
    @ (! [X: $i] :
         ( {$box}
         @ (( ( {$box} @ (member(X,universal_class)) )
           => ( {$box} @ (member(X,singleton(X))) ) )) )) ) ).

...
% Source   : [QMLTP]
% Names    : GSE025+1 [QMLTP]
%          : GSE025_1.047_s5_decreasing_flexible.p [QMLTP/4_NX0_SEMs]
...
%            Number of connectives :  696 (   8   ~;   5   |;  67   &)
%                                         (   0 <=>;  86  =>;   0  <=;   0 <~>)
%                                         ( 530 {}@;   0 {#};   0 {.})
...
tff(ordered_pair_is_set,conjecture,
    ...
    @ (! [X: $i] :
         ( {$box}
         @ (! [Y: $i] : ( {$box} @ (member(ordered_pair(X,Y),universal_class)) )) )) ) ).

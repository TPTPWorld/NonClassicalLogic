...
% Source   : [QMLTP]
% Names    : GSE060+1 [QMLTP]
%          : GSE060_1.047_s5_decreasing_flexible.p [QMLTP/4_NX0_SEMs]

% Status   : Unknown
% Rating   : ? v9.0.0
% Syntax   : Number of formulae    :  116 (   0 unt;  32 typ;   0 def)
%            Number of atoms       : 1304 (   0 equ)
%            Maximal formula atoms :    6 (  15 avg)
%            Number of connectives :  698 (   9   ~;   5   |;  67   &)
%                                         (   0 <=>;  86  =>;   0  <=;   0 <~>)
%                                         ( 531 {}@;   0 {#};   0 {.})
%            Maximal formula depth :    5 (   2 avg)
%            Maximal term depth    :    4 (   1 avg)
%            Number of FOOLs       :  531 ( 531 fml;   0 var)
...
%            Number of variables   :  199 (; 194   !;   5   ?; 199   :)
...
tff(special_classes_lemma,conjecture,
    ( {$box}
    @ (! [X: $i] :
         ( {$box}
         @ (! [Y: $i] :
              ( {$box}
              @ (~ ( {$box} @ (member(Y,intersection(complement(X),X))) )) )) )) ) ).

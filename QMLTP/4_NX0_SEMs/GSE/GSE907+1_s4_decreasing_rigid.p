...
tff(t12_xboole_1, axiom-local, {$box}((! [A:$i]: ({$box}((! [B:$i]: ({$box}(({$box}(subset(A,B)) => {$box}(qmltpeq(set_union2(A,B),B))))))))))).
tff(t38_zfmisc_1, axiom-local, {$box}((! [A:$i]: ({$box}((! [B:$i]: ({$box}((! [C:$i]: (({$box}(({$box}(subset(unordered_pair(A,B),C)) => ({$box}(in(A,C)) & {$box}(in(B,C))))) & {$box}((({$box}(in(A,C)) & {$box}(in(B,C))) => {$box}(subset(unordered_pair(A,B),C))))))))))))))).

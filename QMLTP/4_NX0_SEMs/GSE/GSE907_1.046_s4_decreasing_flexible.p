...
% Source   : [QMLTP]
% Names    : GSE907+1 [QMLTP]
%          : GSE907_1.046_s4_decreasing_flexible.p [QMLTP/4_NX0_SEMs]
...
%            Number of connectives :  164 (   6   ~;   0   |;  10   &)
%                                         (   0 <=>;  18  =>;   0  <=;   0 <~>)
%                                         ( 130 {}@;   0 {#};   0 {.})
    ...
    @ (! [A: $i] :
         ...
         @ (! [B: $i] :
              ...
              @ (! [C: $i] :
                   ...
                   @ (( ( ( {$box} @ (in(A,B)) )
                        & ( {$box} @ (in(C,B)) ) )
                     => ( {$box} @ (qmltpeq(set_union2(unordered_pair(A,C),B),B)) ) )) )) )) )) ) ).

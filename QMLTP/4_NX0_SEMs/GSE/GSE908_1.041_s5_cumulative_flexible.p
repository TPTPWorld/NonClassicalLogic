...
% Source   : [QMLTP]
% Names    : GSE908+1 [QMLTP]
%          : GSE908_1.041_s5_cumulative_flexible.p [QMLTP/4_NX0_SEMs]
...
%            Number of connectives :  190 (   9   ~;   4   |;  11   &)
%                                         (   0 <=>;  25  =>;   0  <=;   0 <~>)
%                                         ( 141 {}@;   0 {#};   0 {.})
...
tff(t49_zfmisc_1,conjecture,
    ...
    @ (! [A: $i] :
         ...
         @ (! [B: $i] :
              ( {$box}
              @ (~ ( {$box} @ (qmltpeq(set_union2(singleton(A),B),empty_set)) )) )) )) ) ).

...
% Source   : [QMLTP]
% Names    : GSE912+1 [QMLTP]
%          : GSE912_1.046_s4_decreasing_flexible.p [QMLTP/4_NX0_SEMs]

% Status   : Unknown
% Rating   : ? v9.0.0
% Syntax   : Number of formulae    :   28 (   0 unt;   6 typ;   0 def)
%            Number of atoms       :  280 (   0 equ)
%            Maximal formula atoms :    2 (  12 avg)
%            Number of connectives :  144 (   2   ~;   0   |;  10   &)
%                                         (   0 <=>;  16  =>;   0  <=;   0 <~>)
%                                         ( 116 {}@;   0 {#};   0 {.})
...
tff(t28_xboole_1,axiom-local,
    ...
    @ (! [A: $i] :
         ...
         @ (! [B: $i] :
              ( {$box}
              @ (( ( {$box} @ (subset(A,B)) )
                => ( {$box} @ (qmltpeq(set_intersection2(A,B),A)) ) )) )) )) ) ).
...
tff(t38_zfmisc_1,axiom-local,
    ...
    @ (! [A: $i] :
         ...
         @ (! [B: $i] :
              ...
              @ (! [C: $i] :
                   ( ( {$box}
                     @ (( ( {$box} @ (subset(unordered_pair(A,B),C)) )
                       => ( ( {$box} @ (in(A,C)) )
                          & ( {$box} @ (in(B,C)) ) ) )) )
                   & ( {$box}
                     @ (( ( ( {$box} @ (in(A,C)) )
    ...
    @ (! [A: $i] :
         ...
         @ (! [B: $i] :
              ...
              @ (! [C: $i] :
                   ...
                   @ (( ( ( {$box} @ (in(A,B)) )
                        & ( {$box} @ (in(C,B)) ) )
                     => ( {$box} @ (qmltpeq(set_intersection2(unordered_pair(A,C),B),unordered_pair(A,C))) ) )) )) )) )) ) ).

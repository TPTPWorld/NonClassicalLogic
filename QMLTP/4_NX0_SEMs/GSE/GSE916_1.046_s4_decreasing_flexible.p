...
tff(t4_xboole_0, axiom-local, {$box} @ ((! [A:$i]: ({$box} @ ((! [B:$i]: (({$box} @ (~ (({$box} @ (~ ({$box} @ (disjoint(A,B)))) & {$box} @ ((! [C:$i]: ({$box} @ (~ ({$box} @ (in(C,set_intersection2(A,B))))))))))) & {$box} @ (~ (((? [C:$i]: ({$box} @ (in(C,set_intersection2(A,B))))) & {$box} @ (disjoint(A,B))))))))))))).

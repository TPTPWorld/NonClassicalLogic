...
tff(l34_zfmisc_1, axiom-local, {$box} @ ((! [A:$i]: ({$box} @ ((! [B:$i]: (({$box} @ (({$box} @ (qmltpeq(set_difference(singleton(A),B),singleton(A))) => {$box} @ (~ ({$box} @ (in(A,B)))))) & {$box} @ (({$box} @ (~ ({$box} @ (in(A,B)))) => {$box} @ (qmltpeq(set_difference(singleton(A),B),singleton(A))))))))))))).

...
tff(l36_zfmisc_1, axiom-local, {$box} @ ((! [A:$i]: ({$box} @ ((! [B:$i]: (({$box} @ (({$box} @ (qmltpeq(set_difference(singleton(A),B),empty_set)) => {$box} @ (in(A,B)))) & {$box} @ (({$box} @ (in(A,B)) => {$box} @ (qmltpeq(set_difference(singleton(A),B),empty_set)))))))))))).

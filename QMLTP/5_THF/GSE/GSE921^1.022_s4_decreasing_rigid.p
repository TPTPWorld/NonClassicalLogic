%------------------------------------------------------------------------------
% File     : GSE921^1.022 : TPTP v8.1.0. Released v8.1.0.
...
%            Number of atoms       :  310 (  10 equ;   0 cnn)
%            Maximal formula atoms :   66 (  10 avg)
%            Number of connectives :  444 (   1   ~;   1   |;   5   &; 431   @)
%                                         (   1 <=>;   5  =>;   0  <=;   0 <~>)
%            Maximal formula depth :   28 (  10 avg)
%            Number of types       :    3 (   1 usr)
%            Number of type conns  :   70 (  70   >;   0   *;   0   +;   0  <<)
%            Number of symbols     :   19 (  18 usr;   1 con; 0-3 aty)
%            Number of variables   :   74 (  61   ^  10   !;   3   ?;  74   :)
% SPC      : TH0_UNK_EQU_NAR
              ...
              @ ^ [B: $i] :
                  ...
                  @ ( mforall_di
                    @ ^ [C: $i] :
                        ( mand
                        @ ( mbox
                          @ ( mimplies @ ( mbox @ ( qmltpeq @ C @ ( set_difference @ A @ B ) ) )
                            @ ( mbox
                              @ ( mforall_di
                                @ ^ [D: $i] : ( mand @ ( mbox @ ( mimplies @ ( mbox @ ( in @ D @ C ) ) @ ( mand @ ( mbox @ ( in @ D @ A ) ) @ ( mbox @ ( mnot @ ( mbox @ ( in @ D @ B ) ) ) ) ) ) ) @ ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( in @ D @ A ) ) @ ( mbox @ ( mnot @ ( mbox @ ( in @ D @ B ) ) ) ) ) @ ( mbox @ ( in @ D @ C ) ) ) ) ) ) ) ) )
                        @ ( mbox
                          @ ( mimplies
                            @ ( mbox
                              @ ( mforall_di
                                @ ^ [D: $i] : ( mand @ ( mbox @ ( mimplies @ ( mbox @ ( in @ D @ C ) ) @ ( mand @ ( mbox @ ( in @ D @ A ) ) @ ( mbox @ ( mnot @ ( mbox @ ( in @ D @ B ) ) ) ) ) ) ) @ ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( in @ D @ A ) ) @ ( mbox @ ( mnot @ ( mbox @ ( in @ D @ B ) ) ) ) ) @ ( mbox @ ( in @ D @ C ) ) ) ) ) ) )
                            @ ( mbox @ ( qmltpeq @ C @ ( set_difference @ A @ B ) ) ) ) ) ) ) ) ) ) ) ) ) ).
        ...
        @ ^ [A: $i] :
            ...
            @ ( mforall_di
              @ ^ [B: $i] :
                  ( mbox
                  @ ( mforall_di
                    @ ^ [C: $i] : ( mand @ ( mbox @ ( mimplies @ ( mbox @ ( in @ A @ ( set_difference @ B @ ( singleton @ C ) ) ) ) @ ( mand @ ( mbox @ ( in @ A @ B ) ) @ ( mbox @ ( mnot @ ( mbox @ ( qmltpeq @ A @ C ) ) ) ) ) ) ) @ ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( in @ A @ B ) ) @ ( mbox @ ( mnot @ ( mbox @ ( qmltpeq @ A @ C ) ) ) ) ) @ ( mbox @ ( in @ A @ ( set_difference @ B @ ( singleton @ C ) ) ) ) ) ) ) ) ) ) ) ) ) ) ).

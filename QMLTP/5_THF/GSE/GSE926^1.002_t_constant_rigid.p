%------------------------------------------------------------------------------
% File     : GSE926^1.002 : TPTP v8.1.0. Released v8.1.0.
...
% Status   : Unknown 
% Rating   : ? v8.1.0
% Syntax   : Number of formulae    :   46 (  11 unt;  19 typ;  10 def)
%            Number of atoms       :  214 (  10 equ;   0 cnn)
%            Maximal formula atoms :   22 (   7 avg)
%            Number of connectives :  302 (   1   ~;   1   |;   2   &; 295   @)
%                                         (   1 <=>;   2  =>;   0  <=;   0 <~>)
%            Maximal formula depth :   19 (   9 avg)
%            Number of types       :    3 (   1 usr)
%            Number of type conns  :   68 (  68   >;   0   *;   0   +;   0  <<)
%            Number of symbols     :   19 (  18 usr;   2 con; 0-3 aty)
%            Number of variables   :   60 (  55   ^   3   !;   2   ?;  60   :)
% SPC      : TH0_UNK_EQU_NAR
    ...
    @ ( mbox
      @ ( mforall_di
        @ ^ [A: $i] :
            ( mbox
            @ ( mforall_di
              @ ^ [B: $i] : ( mand @ ( mbox @ ( mimplies @ ( mbox @ ( qmltpeq @ ( set_difference @ ( singleton @ A ) @ B ) @ ( singleton @ A ) ) ) @ ( mbox @ ( mnot @ ( mbox @ ( in @ A @ B ) ) ) ) ) ) @ ( mbox @ ( mimplies @ ( mbox @ ( mnot @ ( mbox @ ( in @ A @ B ) ) ) ) @ ( mbox @ ( qmltpeq @ ( set_difference @ ( singleton @ A ) @ B ) @ ( singleton @ A ) ) ) ) ) ) ) ) ) ) ) ).
    ...
    @ ( mbox
      @ ( mforall_di
        @ ^ [A: $i] :
            ( mbox
            @ ( mforall_di
              @ ^ [B: $i] : ( mand @ ( mbox @ ( mimplies @ ( mbox @ ( qmltpeq @ ( set_difference @ ( singleton @ A ) @ B ) @ empty_set ) ) @ ( mbox @ ( in @ A @ B ) ) ) ) @ ( mbox @ ( mimplies @ ( mbox @ ( in @ A @ B ) ) @ ( mbox @ ( qmltpeq @ ( set_difference @ ( singleton @ A ) @ B ) @ empty_set ) ) ) ) ) ) ) ) ) ) ).
    ...
    @ ( mbox
      @ ( mforall_di
        @ ^ [A: $i] :
            ( mbox
            @ ( mforall_di
              @ ^ [B: $i] : ( mor @ ( mbox @ ( qmltpeq @ ( set_difference @ ( singleton @ A ) @ B ) @ empty_set ) ) @ ( mbox @ ( qmltpeq @ ( set_difference @ ( singleton @ A ) @ B ) @ ( singleton @ A ) ) ) ) ) ) ) ) ) ).

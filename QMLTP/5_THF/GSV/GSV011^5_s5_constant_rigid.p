...
%%% Generated on Thu Apr 28 14:36:02 EDT 2022
...
thf(ax1, axiom, (mlocal @ (mbox @ (b_holds @ ((key @ (generate_key @ an_a_nonce)) @ a))))).
thf(ax2, axiom, (mlocal @ (mbox @ (message @ (((sent @ a) @ b) @ ((pair @ ((encrypt @ (((triple @ a) @ (generate_key @ an_a_nonce)) @ (generate_expiration_time @ an_a_nonce))) @ bt)) @ ((encrypt @ (generate_b_nonce @ an_a_nonce)) @ (generate_key @ an_a_nonce)))))))).
thf(ax3, axiom, (mlocal @ (mbox @ (a_holds @ ((key @ (generate_key @ an_a_nonce)) @ b))))).
thf(ax4, axiom, (mlocal @ (mbox @ (message @ (((sent @ t) @ a) @ (((triple @ ((encrypt @ ((((quadruple @ b) @ an_a_nonce) @ (generate_key @ an_a_nonce)) @ (generate_expiration_time @ an_a_nonce))) @ at)) @ ((encrypt @ (((triple @ a) @ (generate_key @ an_a_nonce)) @ (generate_expiration_time @ an_a_nonce))) @ bt)) @ (generate_b_nonce @ an_a_nonce))))))).
thf(ax5, axiom, (mlocal @ (mbox @ (message @ (((sent @ b) @ t) @ (((triple @ b) @ (generate_b_nonce @ an_a_nonce)) @ ((encrypt @ (((triple @ a) @ an_a_nonce) @ (generate_expiration_time @ an_a_nonce))) @ bt))))))).
thf(ax6, axiom, (mlocal @ (mbox @ (b_stored @ ((pair @ a) @ an_a_nonce))))).
thf(co1, conjecture, (mlocal @ (mexists_di @ (^ [U:$i]: (((mand @ (mbox @ (a_holds @ ((key @ U) @ b)))) @ (mbox @ (b_holds @ ((key @ U) @ a))))))))).

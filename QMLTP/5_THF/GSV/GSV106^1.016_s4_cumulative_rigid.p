%------------------------------------------------------------------------------
% File     : GSV106^1.016 : TPTP v8.1.0. Released v8.1.0.
...
% Status   : Theorem 
% Rating   : ? v8.1.0
% Syntax   : Number of formulae    :  206 (  12 unt;  58 typ;  10 def)
%            Number of atoms       : 2006 (  10 equ;   0 cnn)
%            Maximal formula atoms :  104 (  13 avg)
%            Number of connectives : 3243 (   1   ~;   1   |;   5   &;3230   @)
%                                         (   1 <=>;   5  =>;   0  <=;   0 <~>)
%            Maximal formula depth :   51 (  15 avg)
%            Number of types       :    3 (   1 usr)
%            Number of type conns  :  113 ( 113   >;   0   *;   0   +;   0  <<)
%            Number of symbols     :   58 (  57 usr;  21 con; 0-4 aty)
...
thf(n_statevars_decl,type,
    n_statevars: $i ).

thf(m_measvars_decl,type,
    m_measvars: $i ).
...
thf(quaternion_ds1_inuse_0018,conjecture,
    ( mlocal
    @ ( mbox
      @ ( mimplies
        @ ( mand
          @ ( mbox
            @ ( mforall_di
              @ ^ [A: $i] : ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( leq @ n0 @ A ) ) @ ( mbox @ ( leq @ A @ ( minus @ m_measvars @ n1 ) ) ) ) @ ( mbox @ ( customqmltpeq @ ( a_select2 @ rho_defuse @ A ) @ use ) ) ) ) ) )
          @ ( mand
            @ ( mbox
              @ ( mforall_di
                @ ^ [B: $i] : ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( leq @ n0 @ B ) ) @ ( mbox @ ( leq @ B @ ( minus @ n_statevars @ n1 ) ) ) ) @ ( mbox @ ( customqmltpeq @ ( a_select2 @ sigma_defuse @ B ) @ use ) ) ) ) ) )
            @ ( mand
              @ ( mbox
                @ ( mforall_di
                  @ ^ [C: $i] :
                      ( mbox
                      @ ( mforall_di
                        @ ^ [D: $i] : ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( leq @ n0 @ C ) ) @ ( mand @ ( mbox @ ( leq @ n0 @ D ) ) @ ( mand @ ( mbox @ ( leq @ C @ n2 ) ) @ ( mbox @ ( leq @ D @ ( minus @ n_steps @ n1 ) ) ) ) ) ) @ ( mbox @ ( customqmltpeq @ ( a_select3 @ u_defuse @ C @ D ) @ use ) ) ) ) ) ) ) )
              @ ( mand
                @ ( mbox
                  @ ( mforall_di
                    @ ^ [E: $i] : ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( leq @ n0 @ E ) ) @ ( mbox @ ( leq @ E @ ( minus @ n_statevars @ n1 ) ) ) ) @ ( mbox @ ( customqmltpeq @ ( a_select2 @ xinit_defuse @ E ) @ use ) ) ) ) ) )
                @ ( mand
                  @ ( mbox
                    @ ( mforall_di
                      @ ^ [F: $i] : ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( leq @ n0 @ F ) ) @ ( mbox @ ( leq @ F @ ( minus @ n_statevars @ n1 ) ) ) ) @ ( mbox @ ( customqmltpeq @ ( a_select2 @ xinit_mean_defuse @ F ) @ use ) ) ) ) ) )
                  @ ( mand
                    @ ( mbox
                      @ ( mforall_di
                        @ ^ [G: $i] : ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( leq @ n0 @ G ) ) @ ( mbox @ ( leq @ G @ ( minus @ n_statevars @ n1 ) ) ) ) @ ( mbox @ ( customqmltpeq @ ( a_select2 @ xinit_noise_defuse @ G ) @ use ) ) ) ) ) )
                    @ ( mbox
                      @ ( mforall_di
                        @ ^ [H: $i] :
                            ( mbox
                            @ ( mforall_di
                              @ ^ [I: $i] : ( mbox @ ( mimplies @ ( mand @ ( mbox @ ( leq @ n0 @ H ) ) @ ( mand @ ( mbox @ ( leq @ n0 @ I ) ) @ ( mand @ ( mbox @ ( leq @ H @ ( minus @ m_measvars @ n1 ) ) ) @ ( mbox @ ( leq @ I @ ( minus @ n_steps @ n1 ) ) ) ) ) ) @ ( mbox @ ( customqmltpeq @ ( a_select3 @ z_defuse @ H @ I ) @ use ) ) ) ) ) ) ) ) ) ) ) ) ) )
        @ ( mbox @ true ) ) ) ) ).

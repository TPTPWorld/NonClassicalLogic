%------------------------------------------------------------------------------
% File     : SET903^1.006 : TPTP v8.1.0. Released v8.1.0.
% Domain   : Set Theory
% Problem  : TPTP problem SET903+1.p with axiomatized equality
% Version  : [BP13] axioms.
% English  : 
...
%            Number of connectives :  219 (   1   ~;   1   |;   2   &; 212   @)
%                                         (   1 <=>;   2  =>;   0  <=;   0 <~>)
%            Maximal formula depth :   18 (   7 avg)
%            Number of types       :    3 (   1 usr)
%            Number of type conns  :   65 (  65   >;   0   *;   0   +;   0  <<)
%            Number of symbols     :   18 (  17 usr;   2 con; 0-3 aty)
%            Number of variables   :   61 (  55   ^   4   !;   2   ?;  61   :)
% SPC      : TH0_THM_EQU_NAR
...
thf(t43_zfmisc_1,axiom,
    ...
    @ ( mforall_di
      @ ^ [A: $i] :
          ...
          @ ^ [B: $i] :
              ( mforall_di
              @ ^ [C: $i] : ( mnot @ ( mand @ ( qmltpeq @ ( singleton @ A ) @ ( set_union2 @ B @ C ) ) @ ( mand @ ( mnot @ ( mand @ ( qmltpeq @ B @ ( singleton @ A ) ) @ ( qmltpeq @ C @ ( singleton @ A ) ) ) ) @ ( mand @ ( mnot @ ( mand @ ( qmltpeq @ B @ empty_set ) @ ( qmltpeq @ C @ ( singleton @ A ) ) ) ) @ ( mnot @ ( mand @ ( qmltpeq @ B @ ( singleton @ A ) ) @ ( qmltpeq @ C @ empty_set ) ) ) ) ) ) ) ) ) ) ) ).
...
thf(t44_zfmisc_1,conjecture,
    ...
    @ ( mforall_di
      @ ^ [A: $i] :
          ...
          @ ^ [B: $i] :
              ( mforall_di
              @ ^ [C: $i] : ( mnot @ ( mand @ ( qmltpeq @ ( singleton @ A ) @ ( set_union2 @ B @ C ) ) @ ( mand @ ( mnot @ ( qmltpeq @ B @ C ) ) @ ( mand @ ( mnot @ ( qmltpeq @ B @ empty_set ) ) @ ( mnot @ ( qmltpeq @ C @ empty_set ) ) ) ) ) ) ) ) ) ) ).

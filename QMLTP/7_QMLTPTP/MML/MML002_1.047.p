%------------------------------------------------------------------------------
% File     : MML002_1.047 : TPTP v9.0.0. Released v9.0.0.
...
%          : MML002_1.047_s5_decreasing_flexible.p [QMLTP/4_NX0_SEMs]

% Status   : Unknown
% Rating   : ? v9.0.0
% Syntax   : Number of formulae    :   54 (   0 unt;   4 typ;   0 def)
%            Number of atoms       :  338 (   0 equ)
%            Maximal formula atoms :    4 (   6 avg)
%            Number of connectives :  240 (  72   ~;   2   |;   0   &)
%                                         (   0 <=>;  46  =>;   0  <=;   0 <~>)
%                                         ( 120 {}@; 120 {#};   0 {.})
...
tff(axiom_t_for_fool_1,axiom-local,
    ! [X: $i] :
      ( ( {$box(#fool)} @ (ws(X)) )
     => ws(X) ) ).

tff(axiom_t_for_fool_2,axiom-local,
    ! [X: $i] :
      ( ( {$box(#fool)} @ (~ ws(X)) )
     => ~ ws(X) ) ).

tff(axiom_4_for_fool_1,axiom-local,
    ! [X: $i] :
      ( ( {$box(#fool)} @ (ws(X)) )
     => ( {$box(#fool)}
        @ ({$box(#fool)} @ (ws(X))) ) ) ).

tff(axiom_4_for_fool_2,axiom-local,
    ! [X: $i] :
      ( ( {$box(#fool)} @ (~ ws(X)) )
     => ( {$box(#fool)}
        @ ({$box(#fool)} @ (~ ws(X))) ) ) ).

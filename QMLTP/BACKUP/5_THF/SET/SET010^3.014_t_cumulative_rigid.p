%------------------------------------------------------------------------------
% File     : SET010^3.014 : TPTP v8.1.0. Released v8.1.0.
% Domain   : Set Theory
% Problem  : TPTP problem SET010+3.p with axiomatized equality
% Version  : [BP13] axioms.
% English  : 
...
% Status   : Theorem 
% Rating   : ? v8.1.0
% Syntax   : Number of formulae    :   59 (  12 unt;  20 typ;  10 def)
%            Number of atoms       :  215 (  10 equ;   0 cnn)
%            Maximal formula atoms :   10 (   5 avg)
%            Number of connectives :  377 (   1   ~;   1   |;   4   &; 366   @)
%                                         (   1 <=>;   4  =>;   0  <=;   0 <~>)
%            Maximal formula depth :   14 (   9 avg)
%            Number of types       :    3 (   1 usr)
%            Number of type conns  :   74 (  74   >;   0   *;   0   +;   0  <<)
%            Number of symbols     :   20 (  19 usr;   1 con; 0-3 aty)
%            Number of variables   :  102 (  92   ^   7   !;   3   ?; 102   :)
% SPC      : TH0_THM_EQU_NAR
...
thf(union_subset,axiom,
    ...
    @ ( mforall_di
      @ ^ [B: $i] :
          ...
          @ ^ [C: $i] :
              ( mforall_di
              @ ^ [D: $i] : ( mimplies @ ( mand @ ( subset @ B @ C ) @ ( subset @ D @ C ) ) @ ( subset @ ( union @ B @ D ) @ C ) ) ) ) ) ) ).
...
thf(intersection_is_subset,axiom,
    ...
    @ ( mforall_di
      @ ^ [B: $i] :
          ( mforall_di
          @ ^ [C: $i] : ( subset @ ( intersection @ B @ C ) @ B ) ) ) ) ).

thf(subset_difference,axiom,
    ( mlocal
    @ ( mforall_di
      @ ^ [B: $i] :
          ( mforall_di
          @ ^ [C: $i] :
              ( mforall_di
              @ ^ [D: $i] : ( mimplies @ ( subset @ B @ C ) @ ( subset @ ( difference @ D @ C ) @ ( difference @ D @ B ) ) ) ) ) ) ) ).
...
thf(prove_difference_and_intersection_and_union,conjecture,
    ( mlocal
    @ ( mforall_di
      @ ^ [B: $i] :
          ( mforall_di
          @ ^ [C: $i] :
              ( mforall_di
              @ ^ [D: $i] : ( qmltpeq @ ( difference @ B @ ( intersection @ C @ D ) ) @ ( union @ ( difference @ B @ C ) @ ( difference @ B @ D ) ) ) ) ) ) ) ).

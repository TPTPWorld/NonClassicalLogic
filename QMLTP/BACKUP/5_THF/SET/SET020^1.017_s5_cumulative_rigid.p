%------------------------------------------------------------------------------
% File     : SET020^1.017 : TPTP v8.1.0. Released v8.1.0.
% Domain   : Set Theory
% Problem  : TPTP problem SET020+1.p with axiomatized equality
% Version  : [BP13] axioms.
% English  : 
...
%            Number of atoms       :  614 (  10 equ;   0 cnn)
%            Maximal formula atoms :   13 (   6 avg)
%            Number of connectives : 1178 (   1   ~;   1   |;   5   &;1165   @)
%                                         (   1 <=>;   5  =>;   0  <=;   0 <~>)
...
thf(unique_1st_and_2nd_in_pair_of_sets1,conjecture,
    ...
    @ ( mforall_di
      @ ^ [U: $i] :
          ...
          @ ^ [V: $i] :
              ( mforall_di
              @ ^ [X: $i] : ( mimplies @ ( mand @ ( member @ U @ universal_class ) @ ( mand @ ( member @ V @ universal_class ) @ ( qmltpeq @ X @ ( ordered_pair @ U @ V ) ) ) ) @ ( mand @ ( qmltpeq @ ( first @ X ) @ U ) @ ( qmltpeq @ ( second @ X ) @ V ) ) ) ) ) ) ) ).
